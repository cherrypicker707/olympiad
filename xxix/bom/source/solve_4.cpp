...
vecvecint_t distance_to_start;
vecvecchar_t direction_to_start;
...
void spread(vecvecint_t &distance, vecvecchar_t &direction);
...
int32_t main()
{
    ...
    distance_to_start.resize(map_size, vecint_t(map_size, LLONG_MAX));
    direction_to_start.resize(map_size, vecchar_t(map_size, '.'));
    set_distance_and_direction(initial_field, distance_to_start, direction_to_start);
    spread(distance_to_start, direction_to_start);
    ...
    spread(distance_to_finish, direction_to_finish);

    for (int_t row = 0; row < map_size; row++)
    {
        for (int_t column = 0; column < map_size; column++)
        {
            if (distance_to_start[row][column] == LLONG_MAX || distance_to_finish[row][column] == LLONG_MAX)
                continue;

            int_t current_distance = distance_to_start[row][column] + distance_to_finish[row][column];
...
void spread(vecvecint_t &distance, vecvecchar_t &direction)
{
    vecvecint_t spread = distance;

    for (int_t row = 0; row < map_size; row++)
    {
        int_t current_distance = LLONG_MAX;
        ...
        for (int_t column = 0; column < map_size; column++)
        {
            ...
            current_distance = std::min(current_distance, distance[row][column]);

            if (current_distance < spread[row][column])
            {
                spread[row][column] = current_distance;
                direction[row][column] = 'L';
            }
    ...
    for (int_t row = 0; row < map_size; row++)
    {
        ...
        for (int_t column = map_size - 1; column >= 0; column--)
        {
            if (map[row][column] == 'X')
            {
                current_distance = LLONG_MAX;
    ...
    for (int_t column = 0; column < map_size; column++)
    {
        ...
        for (int_t row = map_size - 1; row >= 0; row--)
        {
            if (map[row][column] == 'X')
            {
                current_distance = LLONG_MAX;
