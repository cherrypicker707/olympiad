...
void solve_3()
{
    ...
    int_t current_number = find_sequence_element(sequence_fragment_left_bound);
    int_t already_printed = 0;

    while (already_printed < sequence_fragment_count)
    {
        if (std::gcd(current_number, given_number) > 1)
        {
            current_number++;
